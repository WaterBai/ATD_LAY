							...
							$this
									.on(
											...
											function(e) {
												...
												if ($this.attr("href")
														&& Object
																.keys(jcOption.buttons).length == 0) {
													var buttons = {};
	...
	Jconfirm.prototype = {
		...
		_buildHTML : function() {
			...
			template.find(".jconfirm-box").addClass(this.animationParsed)
					.addClass(this.backgroundDismissAnimationParsed).addClass(
							type);
			if (this.typeAnimated) {
				template.find(".jconfirm-box").addClass(
						"jconfirm-type-animated");
			}
			if (this.useBootstrap) {
				template.find(".jc-bs3-row")
						.addClass(this.bootstrapClasses.row);
				template.find(".jconfirm-box-container").addClass(
						this.columnClassParsed);
				if (this.containerFluid) {
					template.find(".jc-bs3-container").addClass(
			...
			template.find(".jconfirm-box").attr("aria-labelledby", ariaLabel)
					.attr("tabindex", -1);
			...
			this.$jconfirmBoxContainer = this.$el
			...
			this.$body.css(this._getCSS(this.animationSpeed,
		...
		_bindEvents : function() {
			...
			this.$scrollPane.click(function(e) {
				if (!that.boxClicked) {
					...
					if (buttonName) {
						var btnResponse = that.buttons[buttonName].action
								.apply(that);
						shouldClose = (typeof btnResponse == "undefined")
								|| !!(btnResponse);
					}
					if (shouldClose) {
						that.close();
		...
		_setButtons : function() {
			...
			$.each(this.buttons, function(key, button) {
				...
				var button_element = $(
						'<button type="button" class="btn '
								...
								+ that.buttons[key].text + "</button>").click(
						function(e) {
							e.preventDefault();
			...
			if (this.closeIcon) {
				if (this.closeIconClass) {
					var closeHtml = '<i class="' + this.closeIconClass
							+ '"></i>';
				...
				if (typeof string == "string") {
					...
					if (typeof string == "function") {
						if (typeof string.promise == "function") {
							console
									.error("Promise was returned from title function, this is not supported.");
						}
						var response = string();
		...
		setContent : function(string, force) {
			force = force || false;
			var that = this;
			this.contentParsed = (typeof string == "undefined") ? this.contentParsed
					: string;
			if (this.isAjaxLoading && !force) {
				return;
			}
		...
		_startCountDown : function() {
			...
			if (opt.length !== 2) {
				console
						.error("Invalid option for autoClose. example 'close|10000'");
				return false;
			}
			var button_key = opt[0];
			var time = parseInt(opt[1]);
			if (typeof this.buttons[button_key] === "undefined") {
				console.error("Invalid button key '" + button_key
						+ "' for autoClose");
				return false;
			}
		...
		reactOnKey : function(e) {
			...
			if (keyChar === "esc" && this.escapeKey) {
				if (this.escapeKey === true) {
					...
					if (typeof this.escapeKey === "string"
		...
		close : function() {
			...
			var closeTimer = (this.closeAnimation == "none") ? 1
					: this.animationSpeed;
			setTimeout(function() {
				that.$el.remove();/* console.log(that._lastFocused); */
				if (that._lastFocused.length
						&& $.contains(document, that._lastFocused[0])) {
